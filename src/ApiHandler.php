<?php
/**
 * EGroupware - RAG - REST API interface
 *
...
class ApiHandler extends Api\CalDAV\Handler
{
	...
	/**
	 * @var string[] search-type => method-name
	 */
	static $types = [
		'hybrid' => 'search',
	...
	function propfind_generator($path, array &$filter, array $extra=[], $nresults=null, $report_not_found_multiget_ids=true)
	{
		...
		{
			$this->filter2col_filter([]);   // throw error
		}
		$search = $filter['type'] ?? self::$types['hybrid'];
		for($chunk=0; ($entries = $this->bo->search($filter['search'], $filter['apps'],
			$chunk*self::CHUNK_SIZE, self::CHUNK_SIZE, true, $filter['order'] ?? 'default ASC')); ++$chunk)
		{
			$titles = [];
			foreach($entries as $id => &$entry)
			{
				$entry[self::$path_attr] = $id;
				[$entry['app'], $entry['app_id']] = explode(':', $id, 2);
				$titles[$entry['app']] ??= [];
				$titles[$entry['app']][] = $entry['app_id'];
			}
			unset($entry);
			foreach($titles as $app => $app_ids)
			{
				...
				// remove entries user has no access to
				foreach ($app_ids as $id)
				{
					unset($entries[$app.':'.$id]);
				}
			}
			foreach($entries as $entry)
			{
				// remove contact from requested multiget ids, to be able to report not found urls
				if (!empty($this->requested_multiget_ids) && ($k = array_search($entry[self::$path_attr], $this->requested_multiget_ids)) !== false)
				{
				...
				if (true)
				{
					$jsobject = ['modified' => Api\CalDAV\JsBase::UTCDateTime($entry['modified'])]+$entry;
					$props['getcontentlength'] = bytes(is_array($jsobject) ? json_encode($jsobject) : $jsobject);
					$props['data'] = Api\CalDAV::mkprop('data', $jsobject);
				}
				if (++$yielded && isset($nresults) && $yielded > $nresults)
				{
					return;
			...
			{
				$this->sync_collection_token = $entry['modified'];
			}
		}
	 ...
	 * - filter[apps][]=<app-name>
	 * - filter[type]= "rag" | "fulltext" | "hybrid" (default)
	 *
	 * @param array $filter
	 * @return array
	 */
	protected function filter2col_filter(array $filter, int $user)
	{
		...
		{
			throw new \Exception("Invalid search pattern filters[search]=".json_encode($filter['search'] ?? null)."!");
		}
		if (isset($filter['type']) && !isset(self::$types[$filter['type']]))
		{
			throw new \Exception("Invalid search-type filters[type]=".json_encode($filter['type'])."!");
		}
		return [
			'search' => $filter['search'],
			'type' => self::$types[$filter['type'] ?? 'hybrid'],
			'apps' => !empty($filter['apps']) ? (array)$filter['apps'] : null,
		];
	}
	...
	protected function handleException(\Throwable $e, ?string $code=null) : string
	{
		...
		elseif (is_a($e, Api\Exception\NotFound::class))
		{
			$e = new \Exception('Not Found', 404, $e);
		}
		elseif (is_a($e, Api\Db\Exception\InvalidSql::class) &&
			preg_match('/^(Duplicate entry.*)$/mi', $e->getMessage(), $matches))// && strpos($e->getMessage(), 'SQLSTATE[HY000]') !== false)
		{
			$e = new \Exception($matches[1], 409, $e);
		}
